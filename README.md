...
혼자 **2주 만에 기획부터 배포까지 완주한 개인 프로젝트**입니다. "완벽주의 대신 완성"을 원칙으로, 과설계 없이 동작하는 서비스를 먼저 만들고 남은 시간에 다듬었습니다.

- **6일 만에 첫 배포** — Day 1 백엔드 세팅에서 Day 6 실배포까지. Render(백엔드) + Vercel(프론트) 무료 티어로 운영.
- **텍스트 · PDF 요약** — Claude API로 요약 / 핵심 포인트 / 액션 아이템을 구조화된 JSON으로 반환. PDF는 PDFBox로 텍스트를 뽑아 같은 흐름으로 처리.
- **실전 문제 해결** — 회사망 SSL 프록시 대응(로컬 · Docker · Gradle 등 JVM별 인증서 설정), LLM이 JSON을 마크다운으로 감싸는 문제 방어, 입력 검증 · 타임아웃 · 에러 계층(400/502/504/500) 정리.
- **AI 페어 개발** — 코딩 에이전트(Kiro-cli)로 구현하고 생성 코드를 직접 검증하는 방식으로 진행. 개발 전 과정을 회고 · 트러블슈팅 문서로 남김.

**역할:** 기획 · 백엔드 · 프론트엔드 · 배포 · 문서화 전 과정 단독 수행
**기간:** 2주 (핵심 기능 완성 → 배포 → 안정화 → 문서화)

> 개발 회고와 AI 활용 경험은 [개발 블로그](docs/blog/retrospective/)에 정리했습니다.
